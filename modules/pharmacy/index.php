...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-6">
        <div>
            <h1 class="text-xl font-bold text-gray-800 tracking-tight italic">Pharmacy Queue</h1>
        ...
        <div class="flex items-center gap-3">
            <div class="px-5 py-2.5 bg-emerald-700 text-white rounded-2xl shadow-lg shadow-emerald-100 flex items-center gap-2 transition-all">
                ...
                <span class="text-xs font-semibold uppercase tracking-wide"><?php echo $pendingCount; ?> Pending TB Regimens</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-all">
        <table class="w-full text-left">
            <thead class="bg-teal-50/50 border-b border-teal-100">
                <tr>
                    <th class="px-8 py-5 text-[10px] font-semibold text-teal-600 uppercase tracking-wide">Patient Profile</th>
                    <th class="px-8 py-5 text-[10px] font-semibold text-teal-600 uppercase tracking-wide">Prescribed Medication</th>
                    <th class="px-8 py-5 text-[10px] font-semibold text-teal-600 uppercase tracking-wide">Request Date</th>
                    <th class="px-8 py-5 text-[10px] font-semibold text-teal-600 uppercase tracking-wide text-right">Action</th>
                </tr>
            </thead>
                        ...
                        <tr class="hover:bg-teal-50/20 transition-all group">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    <div class="w-11 h-11 rounded-2xl bg-teal-600 text-white flex items-center justify-center font-semibold text-sm shadow-md group-hover:scale-110 transition-transform">
                                        <?php echo strtoupper(substr($rx['full_name'], 0, 1)); ?>
                                    </div>
                                    <div>
                                        <p class="font-semibold text-gray-800 text-sm"><?php echo $rx['full_name']; ?></p>
                                        <p class="text-[9px] font-semibold text-teal-400 uppercase tracking-wide mt-1"><?php echo $rx['medical_record_number']; ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <span class="px-3 py-1.5 bg-teal-50 text-teal-700 text-[10px] font-semibold uppercase rounded-lg border border-teal-100 italic">
                            ...
                            <td class="px-8 py-6 text-right">
                                <button type="button"
                                    ...
                                    class="inline-flex items-center gap-2 px-5 py-2.5 bg-teal-600 text-white rounded-xl font-semibold text-[9px] uppercase tracking-widest hover:bg-teal-600 shadow-md transition-all active:scale-95">
                    ...
                    <tr>
                        <td colspan="4" class="p-24 text-center opacity-30">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-xs font-semibold uppercase tracking-wider">No pending dispensations</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
...
<div id="rxModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-md transition-all duration-300">
    <div id="rxCard" class="bg-white rounded-2xl w-full max-w-md p-10 shadow-xl border border-gray-100 transform scale-95 transition-all duration-300">

        <div class="flex justify-between items-start mb-8">
            <div class="w-14 h-14 bg-teal-600 text-white rounded-2xl flex items-center justify-center font-bold shadow-md">Rx</div>
        ...
        <div class="space-y-6 mb-10">
            <div>
                <p class="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Patient Subject</p>
                <h3 id="modalPatientName" class="text-xl font-bold text-gray-800">--</h3>
                <p id="modalMRN" class="text-[10px] font-bold text-teal-500 uppercase tracking-tighter">--</p>
            </div>

            <div class="bg-teal-50 p-6 rounded-xl border border-teal-100">
                <p class="text-[9px] font-semibold text-teal-400 uppercase tracking-wide mb-3">Order Details</p>
                <div class="space-y-4">
                    <div class="flex justify-between border-b border-teal-100/50 pb-2">
                        <span class="text-[10px] font-bold text-teal-600/50 uppercase">Medication</span>
                        <span id="modalMed" class="text-sm font-semibold text-teal-700 uppercase">--</span>
                    </div>
                    <div class="flex justify-between border-b border-teal-100/50 pb-2">
                        <span class="text-[10px] font-bold text-teal-600/50 uppercase">Regimen</span>
                        <span id="modalDose" class="text-sm font-semibold text-teal-700 uppercase">--</span>
                    </div>
                    <!-- NEW: PRESCRIBED BY SECTION -->
                    <div class="flex justify-between">
                        <span class="text-[10px] font-bold text-teal-600/50 uppercase">Prescribed By</span>
                        <span id="modalDoctor" class="text-[10px] font-semibold text-teal-700 uppercase italic">--</span>
                    </div>
                </div>
            </div>
        </div>
