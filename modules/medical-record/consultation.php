...
?>
 ...
 <div class="space-y-6">
    ...
    <div class="sticky -top-10 z-20 bg-white/90 backdrop-blur-md border border-blue-100 p-5 rounded-[2.5rem] shadow-sm flex flex-col gap-4">
        <!-- Notification -->
        <?php if ($msg): ?>
    ...
    <div class="w-14 h-14 bg-blue-600 text-white rounded-full flex items-center justify-center font-bold text-xl shadow-md shrink-0">
    ...
    <form method="POST" id="admitForm" class="inline">
        <button type="button"
            ...
            class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all bg-white text-blue-600 border border-transparent hover:border-blue-200 hover:shadow-sm shadow-sm group">
...
<?php endif; ?>
                ...
                <div class="flex items-center gap-2 bg-gray-50 p-1.5 rounded-2xl border border-gray-100 flex-wrap">
                    ...
                    <?php foreach ($actions as $act):
                        ...
                        if ($viewOnly && $act['id'] != 'discharge'):
                            ...
                            ?>
                            <span
                                class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest opacity-30 grayscale bg-gray-100 text-gray-400 border border-transparent cursor-not-allowed"
                            ...
                            <?php elseif (!$viewOnly): ?>
                                    <a href=" <?php echo $disabled ? '#' : "index.php?page={$act['id']}&id=$id&vid=$visit_id"; ?>"
                                            class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[9px] font-black
                            uppercase tracking-widest transition-all
                            <?php echo $disabled ? 'opacity-20 cursor-not-allowed grayscale bg-gray-100 text-gray-400' : "bg-white text-blue-600 hover:border-blue-200 hover:shadow-sm shadow-sm"; ?>
                     ...
                     <?php endforeach; ?>
                        ...
                        <?php if (!$viewOnly && $hasActiveVisit && $role == 'Doctor'): ?>
                            ...
                            <button type="submit" name="request_nurse"
                                class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest bg-indigo-50 text-indigo-600 border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
    ...
    <?php if ($viewOnly): ?>
        <div
            class="mt-2 bg-amber-50 border border-amber-200 rounded-xl p-3 text-amber-700 text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
    ...
    <?php if (!$hasActiveVisit): ?>
            <div class="p-12 bg-blue-50 border-2 border-dashed border-blue-200 rounded-[3rem] text-center">
                <h3 class="text-blue-700 font-black uppercase tracking-widest mb-2">Check-in Required</h3>
                <p class="text-xs text-blue-500 font-medium max-w-sm mx-auto mb-6">No active visit found.</p>
                <a href="index.php?page=records" class="inline-block px-8 py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg hover:bg-blue-700">Go to Registry</a>
            </div>
    <?php else: ?>
            <div class="grid grid-cols-1 xl:grid-cols-4 gap-8">
                 ...
                 <div class="flex flex-col space-y-8">
                    ...
                    <div class="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm h-fit">
                        <h3 class="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-6 italic border-b border-blue-50 pb-2">Diagnosis Archive</h3>
                        <div class="space-y-6 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            <?php else: ?>
                                    <?php foreach ($history as $h):
                                        ...
                                        ?>
                                            <div class="relative pl-5 border-l-2 border-blue-100 group hover:border-blue-300 transition-colors">
                                                <div class="absolute -left-[5px] top-0 w-2 h-2 bg-blue-500 rounded-full border-2 border-white shadow-sm"></div>
                                                <div class="flex justify-between items-start">
                                                    <p class="text-[9px] font-black text-blue-400 uppercase leading-none">
                                                        <?php echo date("M d, Y", strtotime($h['diagnosis_date'])); ?>
                                                    </p>
                                                    <?php if ($canEdit): ?>
                                                            <div class="relative">
                                                                <button onclick="toggleMenu('dx-<?php echo $h['diagnosis_id']; ?>')" class="text-gray-400 hover:text-blue-600 transition-colors p-1 rounded-full hover:bg-gray-100">
                                                                ...
                                                                <div id="menu-dx-<?php echo $h['diagnosis_id']; ?>" class="hidden absolute right-0 mt-2 w-32 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                                                                    <button onclick="openEditDiagnosisModal('<?php echo $h['diagnosis_id']; ?>', '<?php echo htmlspecialchars($h['diagnosis_details'], ENT_QUOTES); ?>')" class="w-full text-left px-4 py-2 text-[10px] font-bold text-gray-700 hover:bg-blue-50 hover:text-blue-600 flex items-center gap-2">
                                                ...
                                                <p class="text-[11px] font-black text-gray-700 mt-2 leading-relaxed"><?php echo $h['diagnosis_details']; ?></p>
                                            </div>
                                    <?php endforeach; ?>
                            <?php endif; ?>
                        </div>
...
<div class="grid grid-cols-1 gap-8 <?php echo ($role != 'Doctor' && $role != 'Admin') ? 'opacity-40 pointer-events-none' : ''; ?>">
                        <div
                            class="bg-fuchsia-50/50 rounded-[2.5rem] p-8 border border-fuchsia-100 shadow-sm flex flex-col justify-between">
                            <h3 class="text-sm font-black text-gray-800 uppercase italic mb-6 flex items-center gap-2">
                            ...
                            <form method="POST" class="space-y-4">
                                ...
                                <button name="order_radiology_btn"
                                    class="w-full py-3 bg-fuchsia-600 text-white rounded-xl font-black text-[9px] uppercase tracking-widest shadow-lg hover:bg-fuchsia-700">
                                    Transmit Order
                                </button>
                            </form>
                        </div>
...
<?php
    ...
    if (!empty($labResults) || !empty($pendingLabRequests)):
        ?>
        <div class="bg-indigo-600/90 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-indigo-200 relative overflow-hidden animate-in zoom-in duration-500">
            <div class="flex items-center justify-between mb-6 relative z-10">
                <div class="flex items-center gap-3">
                    <h3 class="text-lg font-black uppercase italic tracking-tighter">Laboratory Findings</h3>
                    <?php
                    $totalItems = count($labResults) + count($pendingLabRequests);
                    if ($totalItems > 1):
                        ?>
                            <span class="bg-white/20 text-white text-[8px] font-black px-2 py-1 rounded-full uppercase tracking-wider">
                ...
                <div class="flex items-center gap-2">
                    <?php if (!empty($pendingLabRequests)): ?>
                            <span class="bg-amber-400/20 text-amber-200 text-[8px] font-black px-2 py-1 rounded-full flex items-center gap-1">
                            ...
                            <div class="flex-none w-[300px] snap-start" style="scroll-snap-align: start;">
                                <div class="bg-white/10 p-5 rounded-2xl border border-white/10 backdrop-blur-sm h-full relative overflow-hidden">
                                    ...
                                    <div class="flex justify-between items-center mb-2">
                                        <span class="text-[10px] font-black uppercase tracking-widest text-indigo-200">
                                    ...
                                    <div class="flex items-center gap-2 mb-3">
                                        ...
                                        <span class="text-[8px] font-black uppercase tracking-wider text-amber-200">Results Pending</span>
                                    </div>
                            
                                    <p class="text-sm font-bold text-white/50 italic break-words">Awaiting lab processing...</p>
                            
                                    <?php if ($pending['priority'] == 'STAT'): ?>
                                            <span class="absolute top-2 right-2 text-[8px] font-black bg-red-500 text-white px-2 py-0.5 rounded-full">
                                                STAT
                                            </span>
                                    <?php endif; ?>
                            
                                    <p class="text-[9px] text-indigo-300 mt-3 font-black uppercase tracking-tighter italic">
                    ...
                    <?php foreach ($labResults as $res): ?>
                            <div class="flex-none w-[300px] snap-start" style="scroll-snap-align: start;">
                                <div class="bg-white/10 p-5 rounded-2xl border border-white/10 backdrop-blur-sm h-full">
                                    ...
                                    <!-- Completed indicator -->
                                    <span class="text-[8px] font-black bg-emerald-500/20 text-emerald-200 px-2 py-0.5 rounded-full mb-2 inline-block">
                                        ✓ Results Ready
                                    </span>
                                    ...
                                    <p class="text-[9px] text-indigo-300 mt-3 font-black uppercase tracking-tighter italic truncate">
...
<?php
    ...
    if (!empty($radiologyResults)):
        ?>
        <div
            class="bg-fuchsia-600/90 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-fuchsia-200 relative overflow-hidden animate-in zoom-in duration-500 mt-4">
            <div class="flex items-center justify-between mb-6 relative z-10">
                <div class="flex items-center gap-3">
                    <h3 class="text-lg font-black uppercase italic tracking-tighter">Radiology Findings</h3>
                ...
                <div class="flex overflow-x-auto gap-4 pb-4 scrollbar-thin scrollbar-thumb-transparent scrollbar-track-transparent hover:scrollbar-thumb-white/30"
                    ...
                    <?php foreach ($radiologyResults as $rad): ?>
                        <div class="flex-none w-[350px] snap-start" style="scroll-snap-align: start;">
                            <div class="bg-white/10 p-5 rounded-2xl border border-white/10 backdrop-blur-sm h-full">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-[10px] font-black uppercase tracking-widest text-fuchsia-200">
                                    ...
                                    <?php if ($rad['priority'] == 'STAT'): ?>
                                        <span class="text-[8px] font-black bg-red-500 text-white px-2 py-0.5 rounded-full">STAT</span>
                                    <?php endif; ?>
                                </div>
                                ...
                                <?php if ($rad['findings']): ?>
                                    ...
                                    <?php if ($rad['image_path']): ?>
                                        <a href="<?php echo $rad['image_path']; ?>" target="_blank"
                                            class="inline-flex items-center gap-1 text-[8px] font-black text-fuchsia-200 hover:text-white transition-colors">
                                            <i data-lucide="image" class="w-3 h-3"></i> View Image
                                        </a>
                                    <?php endif; ?>
                                    <p class="text-[8px] text-fuchsia-300 mt-3 font-black uppercase">
                                        Radiologist:
 ...
 <!-- PHYSICIAN PROGRESS NOTES -->
    ...
    <form id="diagnosisForm" method="POST" class="space-y-8">
        ...
        <div class="space-y-3">
            <label class="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-4">Assessment &
                Findings</label>
   ...
   <?php echo $viewOnly ? 'disabled' : 'required'; ?>
        ...
        <?php if ($viewOnly): ?>
            <button type="button" disabled
                class="w-full sm:w-auto px-8 sm:px-14 py-4 bg-gray-400 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest cursor-not-allowed opacity-50">
                Read Only - Discharged
            </button>
        <?php else: ?>
            <button type="submit" name="save_diagnosis"
                class="w-full sm:w-auto px-8 sm:px-14 py-4 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl active:scale-95">
                Sign Record
            </button>
        <?php endif; ?>
        </div>
...
<div
    ...
    <?php if ($diagnosis): ?>
        <form id="treatmentForm" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-black text-emerald-600 uppercase tracking-widest ml-4">Strategy
                    Description</label>
    ...
    <?php else: ?>
        <button type="submit" name="save_treatment"
            class="w-full sm:w-auto px-8 sm:px-14 py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-xl active:scale-95">
            Plan Treatment
        </button>
    <?php endif; ?>
            </div>
    ...
    <?php else: ?>
        <p class="text-center py-6 text-xs font-black uppercase text-gray-300 tracking-[0.3em] italic">Pending Clinical
            Diagnosis</p>
    <?php endif; ?>
</div>
...
<div class="flex justify-between items-start mb-10 relative z-10">
    <div>
        <h4 class="text-[10px] font-black uppercase tracking-[0.3em] text-blue-200">Session Vitals</h4>
        <p class="text-[9px] text-blue-300/60 font-bold uppercase">Current Visit</p>
    </div>

    <?php if ($_SESSION['role'] == 'Nurse' || $_SESSION['role'] == 'Doctor'): ?>
        <a href="index.php?page=record-vital&id=<?php echo $id; ?>&vid=<?php echo $visit_id; ?>"
            class="w-10 h-10 bg-white/20 hover:bg-white text-white hover:text-blue-600 rounded-2xl transition-all flex items-center justify-center shadow-lg backdrop-blur-md group/btn"
...
<!-- TO HERE (Grid starts below) -->
                        <div class="grid grid-cols-1 gap-y-10 relative z-10 font-black">
                            <div><p class="text-[9px] text-blue-200 uppercase mb-1">Temperature</p><p class="text-2xl"><?php echo $vitals['temperature'] ?? '--'; ?>°C</p></div>
                            <div><p class="text-[9px] text-blue-200 uppercase mb-1">Blood Pressure</p><p class="text-2xl"><?php echo $vitals['blood_pressure'] ?? '--/--'; ?></p></div>
                            <div><p class="text-[9px] text-blue-200 uppercase mb-1">Heart Rate</p><p class="text-2xl"><?php echo $vitals['pulse'] ?? '--'; ?> bpm</p></div>
                        </div>
                    </div>

                    <!-- NEW: TREATMENT HISTORY -->
                <div class="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm h-fit">
                    <h3 class="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-6 italic border-b border-emerald-50 pb-2">Treatment Archive</h3>
                        ...
                        <?php else: ?>
                                <?php foreach ($treatmentHistory as $t):
                                    ...
                                    <div class="relative pl-5 border-l-2 border-emerald-100 group hover:border-emerald-300 transition-colors">
                                        ...
                                        <div class="flex justify-between items-start">
                                            <p class="text-[9px] font-black text-emerald-500 uppercase leading-none">
                                                <?php echo date("M d, Y", strtotime($t['start_date'])); ?>
                                            </p>
                                            <div class="flex gap-2">
                                                <button onclick="openDiagnosisViewModal('<?php echo htmlspecialchars($t['diagnosis_details'] ?? 'No diagnosis details available', ENT_QUOTES); ?>', '<?php echo date("M d, Y", strtotime($t['diagnosis_date'] ?? 'now')); ?>')" class="text-blue-400 hover:text-blue-600
                                                    transition-colors p-1 rounded-full hover:bg-blue-50" title="View Associated Diagnosis">
                                                    <i data-lucide="eye" class="w-4 h-4"></i>
                                                </button>
                                        ...
                                        <p class="text-[11px] font-black text-gray-700 mt-2 leading-relaxed"><?php echo $t['description']; ?></p>
                                        <?php if ($t['end_date']): ?>
...
<div id="diagnosisViewModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-[2rem] shadow-2xl p-8 animate-in zoom-in-95 duration-200">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-lg font-black text-gray-800 uppercase italic">Associated Diagnosis</h3>
        ...
        <div class="space-y-6">
            <div class="space-y-2">
                <label class="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-2">Diagnosis Date</label>
                <p id="view_diagnosis_date" class="text-sm font-bold text-gray-700 bg-gray-50 rounded-2xl px-6 py-4 border border-gray-200"></p>
            </div>
            <div class="space-y-2">
                <label class="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-2">Clinical Findings</label>
            ...
            <div class="flex justify-end pt-2">
                <button onclick="closeDiagnosisViewModal()" class="px-8 py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase hover:bg-blue-700 shadow-lg">Close</button>
            </div>
        </div>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-[2rem] shadow-2xl p-8 animate-in zoom-in-95 duration-200">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-lg font-black text-gray-800 uppercase italic">Edit Diagnosis</h3>
        ...
        <form method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-2">Updated Findings</label>
                <textarea name="diagnosis_details" id="edit_diagnosis_details" rows="6" required class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-6 py-4 focus:ring-2 focus:ring-blue-500 font-medium text-gray-700"></textarea>
            </div>
            <div class="flex justify-end gap-3 pt-2">
                <button type="button" onclick="closeEditDiagnosisModal()" class="px-6 py-3 rounded-xl font-black text-[10px] uppercase text-gray-500 hover:bg-gray-100">Cancel</button>
                <button type="submit" name="update_diagnosis" class="px-8 py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase hover:bg-blue-700 shadow-lg">Update Record</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editTreatmentModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-[2rem] shadow-2xl p-8 animate-in zoom-in-95 duration-200">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-lg font-black text-gray-800 uppercase italic">Edit Treatment</h3>
        ...
        <form method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-black text-emerald-600 uppercase tracking-widest ml-2">Strategy Description</label>
            ...
            <div class="flex justify-end gap-3 pt-2">
                <button type="button" onclick="closeEditTreatmentModal()" class="px-6 py-3 rounded-xl font-black text-[10px] uppercase text-gray-500 hover:bg-gray-100">Cancel</button>
                <button type="submit" name="update_treatment" class="px-8 py-3 bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase hover:bg-emerald-700 shadow-lg">Update Plan</button>
            </div>
        </form>
    </div>
</div>
...
<div id="actionModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-[2px] transition-all duration-300">
    <div id="actionCard" class="bg-white rounded-[2.5rem] w-full max-w-[360px] p-8 shadow-2xl border border-gray-100 transform scale-95 transition-all duration-300">
        ...
        <div class="text-center mb-8">
            <h3 id="actionTitle" class="text-xl font-black text-gray-800 tracking-tight mb-2">Confirm Action</h3>
        ...
        <div class="flex gap-3">
            <button onclick="closeActionModal()"
                class="flex-1 py-3 bg-gray-50 text-gray-400 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-gray-100 hover:text-gray-600 transition-all">
                Cancel
            </button>
            <button id="actionConfirmBtn"
                class="flex-1 py-3 bg-blue-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest text-center shadow-lg shadow-blue-100 hover:bg-blue-700 hover:scale-[1.02] transition-all">
                 Confirm
            </button>
        </div>
    </div>
