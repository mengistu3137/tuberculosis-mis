...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-gray-800 tracking-tighter uppercase">Visit History</h1>
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">
                All patient encounters • <?php echo $total_records; ?> total
            </p>
        </div>
        <a href="index.php?page=visit"
            class="px-4 py-2 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all">
            View Active
        </a>
    </div>
            ...
            <tbody class="divide-y divide-gray-50">
                <?php while ($v = $logs->fetch(PDO::FETCH_ASSOC)):
                    ...
                    <tr class="hover:bg-blue-50/20 transition-all group">
                        ...
                        <td class="px-8 py-6">
                            <span
                                class="px-3 py-1.5 <?php echo $bg_style; ?> text-[9px] font-black uppercase rounded-lg border">
                                <?php echo $v['visit_type']; ?>
                            </span>
                        </td>
                        <td class="px-8 py-6">
                            <span
                                class="px-3 py-1.5 bg-<?php echo $statusColor; ?>-100 text-<?php echo $statusColor; ?>-600 text-[9px] font-black uppercase rounded-lg border border-<?php echo $statusColor; ?>-200">
                                <?php echo $statusText; ?>
                            </span>
                        </td>
                        <td class="px-8 py-6">
                            <?php if ($isDischarged): ?>
                                <span class="text-[10px] font-black text-gray-400 bg-gray-100 px-2 py-1 rounded-full">Yes</span>
                            <?php else: ?>
                            ...
                            <?php else: ?>
                                ...
                                <?php if ($_SESSION['role'] == 'Nurse'): ?>
                                    <a href="index.php?page=record-vital&id=<?php echo $v['patient_id']; ?>&vid=<?php echo $v['visit_id']; ?>"
                                        class="p-3 bg-gray-50 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-2xl transition-all inline-block shadow-inner group relative"
                ...
                <div class="flex gap-1.5">
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <a href="index.php?page=visit-history&p=<?php echo $i; ?>"
                            class="w-9 h-9 flex items-center justify-center rounded-xl font-black text-[10px] transition-all <?php echo ($i == $current_page) ? 'bg-blue-600 text-white' : 'bg-white text-gray-400 border border-gray-100'; ?>">
                            <?php echo $i; ?>
                        </a>
                    <?php endfor; ?>
                </div>
